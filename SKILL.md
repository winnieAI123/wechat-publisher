...
description: 'Publish articles to WeChat Official Account (微信公众号) as drafts via API with professional blueprint-tech-style formatting. Use this skill whenever the user asks to "发公众号", "推送到公众号", "发微信文章", "写一篇公众号文章", "写公众号", "公众号排版", "publish to WeChat", "公众号草稿", or mentions creating/writing/publishing content for their WeChat Official Account. This skill handles the FULL workflow — writing Markdown, generating cover image, converting to beautifully formatted WeChat HTML, uploading media, creating draft. ABSOLUTE RULE — you MUST use wechat_draft_publisher.py for ALL WeChat publishing. NEVER construct HTML manually or call WeChat API directly; the script contains inline styling that is REQUIRED for proper formatting. Without the script, articles will appear as ugly unstyled plain text.'
---
...
> The script contains蓝图科技风 inline CSS that is ESSENTIAL for formatting.
